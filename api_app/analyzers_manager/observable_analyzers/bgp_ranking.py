...
class BGPRanking(classes.ObservableAnalyzer):
    ...
    def run(self):
        ...
        try:
            ...
            asn = response.json()['response'][list(response.json()['response'].keys())[0]]['asn']

            to_query = {'asn': asn, 'ipversion': self.address_family, 'limit': 5}
            base_url = 'https://bgpranking-ng.circl.lu'
            r = requests.session().post(urljoin(base_url, 'json/asn'), data=json.dumps(to_query))

            if self.asn_history == True:
                to_query = {"asn": asn, "period": 5}
                r_hist = requests.session().post(urljoin(base_url, 'json/asn_history'), data=json.dumps(to_query))
                results = {
                    "asn": response.json(),
                    "asn_rank": r.json(),
